    ...
    return (
        <Toast onClose={() => onDelete(task.id)} bg={task.active ? "success" : null}>
            ...
            <Form.Group className="m-3 d-inline-flex gap-1">
                <Form.Check name="active" id={`active-${task.id}`} onClick={() => onActive(task.id)} checked={task.active}/>
                <Form.Label htmlFor={`active-${task.id}`}>{task.active ? "En proceso" : "Pendiente"}</Form.Label>
            </Form.Group>

            <Form.Group className="m-3 d-inline-flex gap-1">
                <Form.Check name="completed" id={`completed-${task.id}`} onClick={() => onCompleted(task.id)} checked={task.completed}/>
